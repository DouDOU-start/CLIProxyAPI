...
export const createProviderRecentRequestsCacheController = () => {
  ...
  return {
    forScope(apiBase: string, email: string): ProviderRecentRequestsCache {
      if (apiBase !== currentApiBase || email !== currentEmail) {
...
export function useProviderRecentRequests(options: UseProviderRecentRequestsOptions = {}) {
  ...
  const cache = useMemo(
    () => providerRecentRequestsCacheController.forScope(apiBase, email),
    [apiBase, email]
  );
